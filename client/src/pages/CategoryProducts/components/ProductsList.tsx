...
const ProductsList = () => {
  ...
  const fetchProducts = async () => {
    try {
      ...
      const res = await categoriesAPI.getCategoryProducts(+categoryId, productsPage, limit, "new");
  ...
  useEffect(() => {
    fetchProducts();
  }, [categoryId, productsPage, limit]);

  useEffect(() => {
    navigate(`/categories/${categoryId}/products?page=${productsPage}&limit=${limit}`, {
      replace: true,
    });
  }, [categoryId, productsPage, limit]);
      ...
      {!fetchProductsLoad ? (
        <>
          ...
          {products.length > 0 ? (
            ...
            <div className="flex items-center justify-center px-4 py-12 text-2xl">No Products Found</div>
          )}
        </>
      ) : (
        "loading ..."
