...
const UpdateUserInfo = ({ currentUser }: { currentUser: User }) => {
  const [name, setName] = useState<string>(currentUser.name);
  const [email] = useState<string>(currentUser.email);
  const [phone, setPhone] = useState<string | null | undefined>(currentUser.phone);
  const [country, setCountry] = useState<string | null | undefined>(currentUser.country);
  const [city, setCity] = useState<string | null | undefined>(currentUser.city);
  ...
  const updateUserInfo = async () => {
    try {
      ...
      if (!name || !email) {
        return notify("info", "Account name and email are required");
      }
      ...
      if (!err?.response) {
        ...
        message ? notify("error", message) : notify("error", "Update account fail!");
      }
    } finally {
      ...
      <div className="rounded-sm border border-stroke bg-white shadow-default">
        ...
        <div className="p-7">
          ...
          >
            ...
            <div className="mb-5.5 flex flex-col gap-5.5 sm:flex-row">
              <div className="w-full sm:w-1/2">
                <label className="mb-3 block text-sm font-medium text-black" htmlFor="country">
                  Country
                </label>
                <input
                  className="w-full rounded border border-stroke bg-gray py-3 px-4.5 text-black focus:border-primary focus-visible:outline-none"
              ...
              <div className="w-full sm:w-1/2">
                <label className="mb-3 block text-sm font-medium text-black" htmlFor="city">
                  City
                </label>
                <input
                  className="w-full rounded border border-stroke bg-gray py-3 px-4.5 text-black focus:border-primary focus-visible:outline-none"
            ...
            <div className="mb-5.5">
              ...
              <div className="relative">
                ...
                <input
                  className="w-full rounded border border-stroke bg-gray py-3 pl-11.5 pr-4.5 text-black focus:border-primary focus-visible:outline-none disabled:bg-whiter"
                  type="email"
                  name="emailAddress"
                  id="emailAddress"
            ...
            <div className="flex justify-end gap-4.5">
              <button
                type="submit"
                disabled={updateUserInfoLoad ? true : false}
                style={updateUserInfoLoad ? { opacity: 0.5, cursor: "revert" } : {}}
